...
void i2c_device_set_addr(I2CDevice_t i2c_device, uint8_t device_addr) {
    if (i2c_device == NULL) {
        return ;
    }
...
esp_err_t i2c_read_bytes(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size ,uint8_t *data, uint16_t length) {
    ...
    if (device->reg_bit != I2C_NO_REG) {
        ...
        if (device->reg_bit == I2C_REG_16BIT_LITTLE) {
            i2c_master_write(cmd, (uint8_t *)&reg_addr, out_reg_size, 1);
        } else if(device->reg_bit == I2C_REG_16BIT_BIG) {
            uint16_t swap_data = __builtin_bswap16(reg_addr);
            i2c_master_write(cmd, (uint8_t *)&swap_data, out_reg_size, 1);
        } else {
    ...
    if (err != ESP_OK) {
        log_e("I2C Read Error: 0x%02x, reg: 0x%04x, length: %d, Code: 0x%x", device->addr, reg_addr, length, err);
    } else {
        log_i("I2C Read Success: 0x%02x, reg: 0x%04x, length: %d", device->addr, reg_addr, length);
        log_reg(data, length);
    }
...
esp_err_t i2c_read_bytes_no_stop(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size ,uint8_t *data, uint16_t length) {
    if (i2c_device == NULL || (length > 0 && data == NULL)) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t i2c_read_byte(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size, uint8_t* data) {
    return i2c_read_bytes(i2c_device, reg_addr,out_reg_size ,data, 1);
}

esp_err_t i2c_read_bit(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size, uint8_t *data, uint8_t bit_pos) {
    if (data == NULL) {
        return ESP_FAIL;
    }
    ...
    err = i2c_read_byte(i2c_device, reg_addr,out_reg_size, &bit_data);
    if (err != ESP_OK) {
        return err;
    }
...
esp_err_t i2c_read_bits(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size ,uint8_t *data, uint8_t bit_pos, uint8_t bit_length) {
...
esp_err_t i2c_write_bytes(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size, uint8_t *data, uint16_t length) {
    if (i2c_device == NULL || (length > 0 && data == NULL)) {
        return ESP_FAIL;
    }
    ...
    if (device->reg_bit != I2C_NO_REG) {
        if (device->reg_bit == I2C_REG_16BIT_LITTLE) {
            i2c_master_write(write_cmd, (uint8_t *)&reg_addr, out_reg_size, 1);
        } else if(device->reg_bit == I2C_REG_16BIT_BIG) {
            uint16_t swap_data = __builtin_bswap16(reg_addr);
            i2c_master_write(write_cmd, (uint8_t *)&swap_data, out_reg_size, 1);
        } else {
    ...
    if (err != ESP_OK) {
        log_e("I2C Write Error, addr: 0x%02x, reg: 0x%04x, length: %d, Code: 0x%x", device->addr, reg_addr, length, err);
    } else {
        log_i("I2C Write Success, addr: 0x%02x, reg: 0x%04x, length: %d", device->addr, reg_addr, length);
        if (length) {
            log_reg(data, length);
        }
    }
...
esp_err_t i2c_write_byte(I2CDevice_t i2c_device, uint32_t reg_addr, size_t out_reg_size,uint8_t data) {
    return i2c_write_bytes(i2c_device, reg_addr,out_reg_size, &data, 1);
}

esp_err_t i2c_write_bit(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size ,uint8_t data, uint8_t bit_pos) {
    uint8_t value = 0x00;
    esp_err_t err = ESP_FAIL;
    err = i2c_read_byte(i2c_device, reg_addr,out_reg_size ,&value);
    if (err != ESP_OK) {
        return err;
    }
    ...
    return i2c_write_byte(i2c_device, reg_addr,out_reg_size, value);
}

esp_err_t i2c_write_bits(I2CDevice_t i2c_device, uint32_t reg_addr,size_t out_reg_size ,uint8_t data, uint8_t bit_pos, uint8_t bit_length) {
