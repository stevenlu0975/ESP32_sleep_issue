...
esp_err_t gt911_i2c_read(uint16_t register_addr, uint8_t *data_buf, uint8_t len) {
    return i2c_read_bytes(gt911_device, register_addr,GT911_OUT_REG_SIZE, data_buf, len);
}

esp_err_t gt911_i2c_write8(uint16_t register_addr, uint8_t data) {
    uint8_t buffer = data;
    return i2c_write_bytes(gt911_device, register_addr,GT911_OUT_REG_SIZE ,&buffer, 1);
}
    ...
    if (!gt911_status.inited) {
        ...
        if ((ret = gt911_i2c_read(GT911_PRODUCT_ID1, &data_buf, 1) != ESP_OK)) {
            // ESP_LOGE(TAG, "Error reading from device: %s",
             ...
             ESP_LOGE(TAG, "Error reading from device: gt911");    // Only show error the first time        
            return ret;
        }
        ...
        ESP_LOGE(TAG, "\t GT911 Touch init inited = false");
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "\t GT911 Touch init success");
    return ESP_OK;
}
